...
use iced::widget::{
    Button, Container, PickList, Radio, Row, Scrollable, Slider, Space, Svg, Text, column as col,
    row,
};
...
impl TacticalThemes {
    const ALL: [TacticalThemes; 73] = [
        ...
        TacticalThemes::BalestraMate,
        TacticalThemes::BlindSwineMate,
        TacticalThemes::BodenMate,
        TacticalThemes::CornerMate,
        TacticalThemes::DoubleBishopMate,
        TacticalThemes::DovetailMate,
        TacticalThemes::EpauletteMate,
        TacticalThemes::HookMate,
        TacticalThemes::KillBoxMate,
        TacticalThemes::PillsburyMate,
        TacticalThemes::MorphysMate,
        TacticalThemes::OperaMate,
        TacticalThemes::SwallowstailMate,
        TacticalThemes::TriangleMate,
...
fn adapt_sqlite_puzzle(puzzle: chess_material_studio::models::Puzzle) -> config::Puzzle {
    config::Puzzle {
...
fn search_csv_from_path(
    ...
    for result in reader.deserialize::<config::Puzzle>() {
        let record = result.map_err(|error| {
            format!(
                "cannot deserialize CSV row in {}: {error}",
                csv_path.display()
            )
        })?;
...
impl SearchTab {
    pub fn new() -> Self {
        SearchTab {
            ...
            opening: PickListWrapper::new_opening(
                config::SETTINGS.lang,
                config::SETTINGS.last_opening,
            ),
            variation: PickListWrapper::new_variation(
                config::SETTINGS.lang,
                config::SETTINGS.last_variation.clone(),
            ),
    ...
    pub fn update(&mut self, message: SearchMesssage) -> Task<Message> {
        ...
        match message {
            ...
            SearchMesssage::ClickSearch => Task::none(),
    ...
    pub fn start_lichess_search(
        &mut self,
        generation: u64,
        excluded_ids: HashSet<String>,
    ) -> Result<Task<Message>, &'static str> {
    ...
    fn content(&self) -> Element<'_, Message> {
        let mut search_col = col![
            Container::new(
                row![
                    Radio::new(
                        lang::tr(&self.lang, "lichess_db"),
                        SearchBase::Lichess,
                        self.base,
                        SearchMesssage::SelectBase
                    )
                    .style(styles::radio_style),
                    Radio::new(
                        lang::tr(&self.lang, "my_favories"),
        ...
        if self.opening.item != Openings::Any {
            let row_color = row![
                Radio::new(
                    lang::tr(&self.lang, "any"),
                    OpeningSide::Any,
                    self.opening_side,
                    SearchMesssage::SelectOpeningSide
                )
                .style(styles::radio_style),
                Radio::new(
            ...
            search_col = search_col
                .push(Text::new(lang::tr(&self.lang, "side")))
                .push(row_color);
        }
        ...
        search_col = search_col
            .push(
                Button::new(Text::new(lang::tr(&self.lang, "btn_search")))
                    .padding(5)
                    .on_press(SearchMesssage::ClickSearch)
...
mod tests {
    ...
    fn setup_test_sqlite() -> std::path::PathBuf {
        ...
        chess_material_studio::puzzle_import::import_puzzles_from_reader(
            &mut conn,
            FIXTURE.as_bytes(),
        )
    ...
    fn exclusions_filter_and_truncate_without_decision_details() {
        ...
        let excluded =
            std::collections::HashSet::from(["selected".to_owned(), "discarded".to_owned()]);

        let filtered = filter_excluded_puzzles(candidates.clone(), &excluded, 2);
        assert_eq!(
            filtered
                .into_iter()
                .map(|puzzle| puzzle.puzzle_id)
                .collect::<Vec<_>>(),
            ["new-one", "new-two"]
        );
        assert_eq!(
            filter_excluded_puzzles(candidates, &std::collections::HashSet::new(), 3).len(),
            3
        );
    }
    ...
    fn test_parity_all() {
        ...
        let csv_ids: std::collections::HashSet<&str> =
            csv_results.iter().map(|p| p.puzzle_id.as_str()).collect();
        let sqlite_ids: std::collections::HashSet<&str> = sqlite_results
            .iter()
            .map(|p| p.puzzle_id.as_str())
            .collect();
